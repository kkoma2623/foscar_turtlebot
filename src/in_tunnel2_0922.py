...
def callback1(data):
	...
	wall = 0.2		# minimum value of distance from wall turtlebot have to aviod 
	scan_wall2(Lidar2)
				# Consider present position is visited place or not
	# print(dot)
	# print(count)
	if stage == 3 and first ==1:
		...
		if scan_wall(Lidar3) == 1:		# If there is wall in front of turtlebot or block way, turn back
			...
			if bool_way[0] == 1:		# If there is wall beside the turtlebot
				...
				if Lidar[0] <= wall+0.05 and Lidar[0] > 0: 	# If turtlebot is in uncertain position with blocked wall
					store_pose()
					remind_past()
			
			
					if Lidar[1] > Lidar[2]:		# Compare the left and right side of distance and turn to long distance
						while check<= 100:
							destination5(1)
...
def callback3(data):
	...
	if stage==3: 	
		...
		angle_goal = math.degrees(atan((1.66-local_pose[1])/(2+local_pose[0])))
...
def callback_sotp(frame):
    HSL_RED_LOWER = np.array([0, 50, 50])
    HSL_RED_UPPER = np.array([17, 255, 255])
    cv_image = bridge.compressed_imgmsg_to_cv2(frame, "bgr8")
    ROI = cv_image[50:300, 220:330]
    hsv = cv2.cvtColor(ROI, cv2.COLOR_BGR2HSV)
    red_binary = cv2.inRange(hsv, HSL_RED_LOWER, HSL_RED_UPPER)
	# cv2.imshow('ori', ROI)
    cv2.imshow('red', red_binary)
    nonzero = cv2.countNonZero(red_binary)
    # print nonzero
    cv2.waitKey(1)
    if nonzero > 200 :
		pass
        # os.system('rosrun foscar_turtlebot3_autorace linetrace_0921')
        # os.system('rosnode kill /vel')

def callback5(data):
	msg = data
	# print 'aaaaaaaaaaaaaaaaaaaaaaa',msg.ranges[90],  msg.ranges[270]
	if (2 < msg.ranges[0] or msg.ranges[0] == 0) and 2 < msg.ranges[270] or msg.ranges[270] == 0 :
		os.system('rosrun foscar_turtlebot3_autorace linetrace_0921')
...
def listen():
	...
	# rospy.Subscriber('/camera2/usb_cam2/image_raw/compressed',CompressedImage, callback_sotp,  queue_size = 1)
